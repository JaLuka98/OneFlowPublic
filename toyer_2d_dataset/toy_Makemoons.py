...
def main():
    ...
    # generating out toy data and simulation
    data, labels  = make_moons(n_samples = 1000000, noise=0.05)
    ...
    torch.save(test_inputs, 'Make_moons_inputs.pt')
    torch.save(test_conditions, 'Make_moons_conditions.pt')
    ...
    # The training keeps going for a long time, so lets add a limit of 125 epochs since the results are already quite good ...
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            #scheduler.step( val_loss.mean() )     #comment this for cossine annealing
            ...
            #plotting the loss function of the training process
            training_utils.loss_plot(val_loss_array, train_loss, model='Make_moons')

            # Saving the flow model for each state
            torch.save(flow.state_dict() , 'saved_models/model_' + str(epoch) + '.pth' )
            if(early_stopper.early_stop(float( val_loss.mean() )) ):
                print( 'Early stop criterion achieved!' )
                break
    ...
    torch.save(flow.state_dict() , 'model_make_moons.pth' )
